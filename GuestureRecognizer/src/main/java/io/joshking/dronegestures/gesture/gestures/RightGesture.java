...
import io.joshking.dronegestures.drone.commands.command.RightCommand;
import io.joshking.dronegestures.gesture.Gesture;
import io.joshking.dronegestures.gesture.Hand;

public class RightGesture implements Gesture {
    private final Hand    hand;
    private final Point   point;
    private       boolean hasSentCommand;

    public RightGesture(Point point, Hand hand) {
        this.point = point;
        this.hand = hand;
    }
    ...
    public void update(Hand hand, DroneController drone) {
        if (!hasSentCommand) {
            drone.sendCommand(new RightCommand());
        }
