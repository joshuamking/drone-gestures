...
import io.joshking.dronegestures.drone.commands.command.ForwardCommand;
import io.joshking.dronegestures.gesture.Gesture;
import io.joshking.dronegestures.gesture.Hand;

public class ForwardGesture implements Gesture {
    private final Hand    hand;
    private final Point   point;
    private       boolean hasSentCommand;

    public ForwardGesture(Point point, Hand hand) {
        this.point = point;
        this.hand = hand;
    }
    ...
    public void update(Hand hand, DroneController drone) {
        if (!hasSentCommand) {
            drone.sendCommand(new ForwardCommand());
        }
